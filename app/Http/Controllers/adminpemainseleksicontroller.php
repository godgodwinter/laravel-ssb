...
class adminpemainseleksicontroller extends Controller
{
    ...
    public function storeangkatan(tahunpenilaian $tahunpenilaian, Request $request)
    {
        ...
        // dd($request->angkatan, $ambildatapemain);
        foreach ($ambildatapemain as $item) {
            $pemainid = $item->id;
            //peiksa jika belum ada maka insert
            $periksa = pemainseleksi::where('pemain_id', $pemainid)
                ->where('tahunpenilaian_id', $tahunpenilaian->id)
                ->count();

            // dd($request->angkatan, $ambildatapemain, $periksa);
            if ($periksa < 1) {
                $data_id = pemainseleksi::insertGetId(
                    array(
                        'pemain_id'     =>   $pemainid,
                        'tahunpenilaian_id'     =>   $tahunpenilaian->id,
                        'created_at' => date("Y-m-d H:i:s"),
