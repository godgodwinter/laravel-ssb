...
class adminpenilaiancontroller extends Controller
{
    ...
    public function penilaianpemaininputstore(tahunpenilaian $tahunpenilaian, pemainseleksi $pemain, prosespenilaian $prosespenilaian, Request $request)
    {
        ...
        // dd($tahunpenilaian, $pemain, $prosespenilaian->id);
        // dd($request);
        ...
        foreach ($datakriteria as $kriteria) {
            ...
            foreach ($datakriteriadetail as $dkd) {
                // dd($request[$dkd->id], $dkd->id);
                if ($request[$dkd->id] != 0) {
                    $periksaData = penilaian::where('pemainseleksi_id', $pemain->id)->where('kriteriadetail_id', $dkd->id);
                    if ($periksaData->count() < 1) {
                        penilaian::insert([
                            'pemainseleksi_id' => $pemain->id,
                            'kriteriadetail_id' => $dkd->id,
                    ...
                    // } else {
                    // }
                    $periksa = penilaiandetail::where('prosespenilaian_id', $prosespenilaian->id)->where('penilaian_id', $getpenilaian_id->id)->count();
                    // dd($periksa, $prosespenilaian->id, $getpenilaian_id, $pemain->id, $dkd->id);
                    // dd($periksa);
                    if ($periksa > 0) {
                        $getData = penilaiandetail::where('prosespenilaian_id', $prosespenilaian->id)->where('penilaian_id', $getpenilaian_id->id)->first();
                        // dd($getData);
                        // delete
                        // DB::table('penilaiandetail')->where('id', $getData->id)->delete();
                        // penilaiandetail::insert([
                        //     'prosespenilaian_id' => $prosespenilaian->id,
                        //     'penilaian_id' => $getpenilaian_id->id,
                        //     'nilai' => $request[$dkd->id],
                        // ]);
                        DB::table('penilaiandetail')->where('id', $getData->id)->update([
                            'nilai' => $request[$dkd->id],
                        ]);
                        // dd($getData);
                        // penilaiandetail::where('prosespenilaian_id', $prosespenilaian->id)->where('penilaian_id', $getpenilaian_id->id)
                        //     ->update([
                        //         'nilai'     =>   $request[$dkd->id]
                        //     ]);
                        // // dd($getpenilaian_id->id, $prosespenilaian->id, $request[$dkd->id]);
                        // //
                    } else {
                        penilaiandetail::insert([
                            'prosespenilaian_id' => $prosespenilaian->id,
